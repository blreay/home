...
# This script can't work correctly after 2018 festival holiday
# SSO login success, but 404 is returned.

#typeset txturl="$SSO_WIFI_PWD_URL"
typeset txturl="http://gmp.oracle.com/captcha/files/airespace_pwd_apac.txt"
...
typeset curlcmd="curl -v -L -A \"${useragent}\"  --retry 2 --retry-delay 1 --retry-max-time $curl_timeout --max-time $curl_timeout --insecure -c $cookiefn -b $cookiefn "
#typeset curlcmd="curl -v -L -A \"${useragent}\"  --retry 2 --retry-delay 1 --retry-max-time $curl_timeout --max-time $curl_timeout -c $cookiefn -b $cookiefn "
